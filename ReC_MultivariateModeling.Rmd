...
```{r recoding factor labels, warning=FALSE, message=FALSE}
library(dplyr)
scrub_df <- scrub_df %>%
...
At the scale-level, this is much easier to interpret.  As I am lecturing the results today (the numbers will change) there are *4* rows of data because there are only *4* patterns of missingness. The most common pattern is non-missing data (*n* = 25). 
...
library(formattable)
Bel_B <- digits(Climate_fit$coefficients[2],3)
Bel_p <- digits(summary(Climate_fit)$coefficients[2,4],3)
Bel_B
Bel_p

cm_B <- digits(Climate_fit$coefficients[3],3)
cm_p <- digits(summary(Climate_fit)$coefficients[3,4],3)
cm_B
cm_p

i_B <- digits(Climate_fit$coefficients[4],3)
i_p <- digits(summary(Climate_fit)$coefficients[4,4],3)
i_B
i_p

```

Results of a multiple regression predicting the respondents' perceptions of campus climate for Black students indicated that neither contributions of the respondents' personal belonging ($B$ = `r Bel_B`, $p$ = `r Bel_p`), the proportion of classmates who were Black ($B$ = `r cm_B`, $p$ = `r cm_p`), nor the proportion of BIPOC instructional staff ($B$ = `r i_B`, $p$ = `r i_p`) led to statistically significant changes in perceptions of campus climate for Black students. Results are presented in Table 2.
