...
\textbf{If you are viewing this document, you should know that this is a book-in-progress. Early drafts are released for the purpose teaching my classes and gaining formative feedback from a host of stakeholders. The document was last updated on 04 Apr 2021}
...
CAPTURING RESULTS FOR WRITING IT UP: Data screening suggested that 34 individuals opened the survey link.
...
CAPTURING RESULTS FOR WRITING IT UP: Data screening suggested that 34 individuals opened the survey link. Of those, 32, granted consent and proceeded into the survey items.
...
CAPTURING RESULTS FOR WRITING IT UP: Data screening suggested that 34 individuals opened the survey link. Of those, 32, granted consent and proceeded into the survey items. A further inclusion criteria was that the course was taught in the U.S; 32 met this criteria.
...
Data screening suggested that 34 individuals opened the survey link. Of those, 32 granted consent and proceeded to the survey items. A further inclusion criteria was that the course was taught in the U.S; 32 met this criteria.
...
## X1    1 32 3.98 10.05      0    1.05   0   0 36.36 36.36 2.44     4.67 1.78
\end{verbatim}
...
CUMULATIVE CAPTURE FOR WRITING IT UP: Across cases that were deemed eligible on the basis of the inclusion/exclusion criteria, missingness ranged from 0\% to 36\%. Across the dataset, 3.98\% of cells had missing data and 81.25\% of cases had nonmissing data.
...
\begin{verbatim}
##    vars  n mean    sd median trimmed mad min   max range skew kurtosis   se
## X1    1 32 4.17 10.37      0    1.28   0   0 33.33 33.33 2.14     3.02 1.83
\end{verbatim}
...
CUMULATIVE CAPTURE FOR WRITING IT UP: Across the 32 cases for which the scoring protocol was applied, missingness ranged from 0\% to 33\%.
...
CUMULATIVE CAPTURE FOR WRITING IT UP: Across the 32 cases for which the scoring protocol was applied, missingness ranged from 0\% to 33\%. After eliminating cases with greater than 20\% missing, the dataset analyzed included 29 cases.
...
CUMULATIVE CAPTURE FOR WRITING IT UP: Across the 32 cases for which the scoring protocol was applied, missingness ranged from 0\% to 33\%. After eliminating cases with greater than 20\% missing, the dataset analyzed included 29 cases. In this dataset we had 1.15\% missing across the df; 93.10\% of the rows had nonmissing data.
...
Across cases that were deemed eligible on the basis of the inclusion/exclusion criteria, missingness ranged from 0\% to 36\%. Across the dataset, 3.98\% of cells had missing data and 81.25\% of cases had nonmissing data. At this stage in the analysis, we allowed all cases with less than 90\% missing to continue to the scoring stage. Guided by Parent's \citeyearpar{parent_handling_2013} AIA approach, scales with three items were scored if at least two items were non-missing; the scale with four items was scored if it at least three non-missing items; and the scale with six items was scored if it had at least five non-missing items.

Across the 32 cases for which the scoring protocol was applied, missingness ranged from 0\% to 33\%. After eliminating cases with greater than 20\% missing, the dataset analyzed included 29 cases. In this dataset we had 1.15\% missing across the df; 93.10\% of the rows had nonmissing data.
...
##       0.95      0.95    0.94      0.87  21 0.015  4.2 1.5     0.88
## 
...
## Belong_1      0.93      0.93    0.88      0.88 14.2    0.023    NA  0.88
## Belong_2      0.90      0.90    0.82      0.82  8.9    0.036    NA  0.82
## Belong_3      0.96      0.96    0.93      0.93 24.8    0.014    NA  0.93
## 
##  Item statistics 
##           n raw.r std.r r.cor r.drop mean  sd
## Belong_1 31  0.95  0.96  0.93   0.90  4.3 1.6
## Belong_2 32  0.97  0.98  0.97   0.94  4.4 1.6
## Belong_3 31  0.94  0.94  0.88   0.86  4.0 1.6
## 
## Non missing response frequency for each item
##             1    2    3    4    5    6    7 miss
## Belong_1 0.03 0.10 0.23 0.16 0.23 0.19 0.06 0.03
## Belong_2 0.03 0.06 0.25 0.19 0.16 0.25 0.06 0.00
## Belong_3 0.06 0.16 0.13 0.23 0.26 0.10 0.06 0.03
\end{verbatim}

CAPTURE FOR THE APA STYLE WRITE-UP(since these are generally placed in the description of each measure in the Method section, I am not doing a cumulative capture): Cronbach's alpha for the belonging scale was 0.953.
...
## 
##   raw_alpha std.alpha G6(smc) average_r S/N   ase mean sd median_r
##       0.84      0.86    0.89      0.51 6.2 0.044  2.5  1     0.52
## 
##  lower alpha upper     95% confidence boundaries
## 0.76 0.84 0.93 
## 
##  Reliability if an item is dropped:
##         raw_alpha std.alpha G6(smc) average_r S/N alpha se var.r med.r
## rBlst_1      0.84      0.86    0.89      0.55 6.2    0.047 0.034  0.58
## Blst_2       0.85      0.86    0.87      0.55 6.1    0.045 0.031  0.57
## Blst_3       0.85      0.87    0.89      0.57 6.6    0.043 0.024  0.57
## Blst_4       0.80      0.82    0.85      0.48 4.5    0.059 0.035  0.46
## Blst_5       0.78      0.81    0.84      0.47 4.4    0.062 0.036  0.48
## Blst_6       0.79      0.80    0.80      0.44 4.0    0.060 0.025  0.46
## 
##  Item statistics 
##          n raw.r std.r r.cor r.drop mean   sd
## rBlst_1 28  0.67  0.68  0.58   0.51  3.3 1.30
## Blst_2  31  0.73  0.68  0.61   0.54  2.9 1.69
## Blst_3  31  0.64  0.64  0.56   0.46  2.2 1.34
## Blst_4  30  0.84  0.84  0.83   0.74  2.4 1.19
## Blst_5  31  0.88  0.86  0.85   0.79  2.1 1.24
## Blst_6  31  0.87  0.91  0.93   0.85  2.1 0.99
## 
## Non missing response frequency for each item
##            1    2    3    4    5    6 miss
## rBlst_1 0.07 0.21 0.29 0.29 0.07 0.07 0.12
## Blst_2  0.32 0.16 0.13 0.13 0.23 0.03 0.03
## Blst_3  0.39 0.35 0.06 0.13 0.03 0.03 0.03
## Blst_4  0.23 0.37 0.20 0.13 0.07 0.00 0.06
## Blst_5  0.45 0.26 0.10 0.16 0.03 0.00 0.03
## Blst_6  0.29 0.42 0.16 0.13 0.00 0.00 0.03
\end{verbatim}

CAPTURE FOR THE APA STYLE WRITE-UP (since these are generally placed in the description of each measure in the Method section, I am not doing a cumulative capture): Cronbach's alpha for the campus climate scale was 0.843. Since this value is \(\geq\) .80, it is within the realm of acceptability. Let's go ahead, though, and look at its subscales.
...
## 
##   raw_alpha std.alpha G6(smc) average_r S/N   ase mean  sd median_r
##        0.7      0.72    0.71      0.46 2.6 0.094  2.4 1.2      0.5
## 
##  lower alpha upper     95% confidence boundaries
## 0.52 0.7 0.88 
## 
##  Reliability if an item is dropped:
##        raw_alpha std.alpha G6(smc) average_r  S/N alpha se var.r med.r
## Blst_3      0.78      0.80    0.67      0.67 4.01    0.072    NA  0.67
## Blst_2      0.67      0.67    0.50      0.50 2.01    0.117    NA  0.50
## Blst_5      0.34      0.35    0.21      0.21 0.54    0.226    NA  0.21
## 
##  Item statistics 
##         n raw.r std.r r.cor r.drop mean  sd
## Blst_3 31  0.69  0.71  0.49   0.37  2.2 1.3
## Blst_2 31  0.82  0.78  0.67   0.50  2.9 1.7
## Blst_5 31  0.90  0.90  0.87   0.76  2.1 1.2
## 
## Non missing response frequency for each item
##           1    2    3    4    5    6 miss
## Blst_3 0.39 0.35 0.06 0.13 0.03 0.03 0.03
...
Cronbach's alpha for the campus climate stigma subscale was 0.700.
...
## 
##   raw_alpha std.alpha G6(smc) average_r S/N   ase mean sd median_r
##       0.84      0.85    0.81      0.65 5.6 0.052  2.6  1      0.6
## 
##  lower alpha upper     95% confidence boundaries
## 0.73 0.84 0.94 
## 
##  Reliability if an item is dropped:
##         raw_alpha std.alpha G6(smc) average_r S/N alpha se var.r med.r
## rBlst_1      0.88      0.89    0.81      0.81 8.3    0.039    NA  0.81
## Blst_4       0.73      0.75    0.60      0.60 3.0    0.090    NA  0.60
## Blst_6       0.71      0.71    0.55      0.55 2.4    0.103    NA  0.55
## 
##  Item statistics 
##          n raw.r std.r r.cor r.drop mean   sd
## rBlst_1 28  0.83  0.82  0.64   0.60  3.3 1.30
## Blst_4  30  0.89  0.90  0.85   0.74  2.4 1.19
## Blst_6  31  0.89  0.92  0.88   0.79  2.1 0.99
## 
## Non missing response frequency for each item
##            1    2    3    4    5    6 miss
## rBlst_1 0.07 0.21 0.29 0.29 0.07 0.07 0.12
...
Cronbach's alpha for the campus climate responsiveness subscale was 0.836. Between the two subscales, it looks as if the responsivenes subscale is more internally consistent.
...
## iBIPOC_pr    1 30 0.42 0.40   0.33    0.40 0.49   0  1.00  1.00  0.44    -1.39
## cmBlack      2 32 7.53 8.61   5.00    6.23 7.41   0 29.00 29.00  1.10    -0.07
## Belonging    3 31 4.24 1.52   4.00    4.28 1.97   1  7.00  6.00 -0.19    -0.93
## ClimateBL    4 29 2.44 0.99   2.33    2.41 0.99   1  4.17  3.17  0.15    -1.15
##             se
## iBIPOC_pr 0.07
## cmBlack   1.52
## Belonging 0.27
...
## W = 0.94983, p-value = 0.1812
\end{verbatim}

CUMULATIVE CAPTURE FOR THE APA STYLE WRITE-UP: Regarding the distributional characteristics of the data, skew and kurtosis values of the variables fell below the values of 3 (skew) and 8 to 20 (kurtosis) that Kline suggests are concerning \citeyearpar{kline_principles_2016}. Results of the Shapiro-Wilk test of normality indicate that our variables assessing the proportion of classmates who are Black (\(W\) = 0.811, \(p\) = 0.0001) and the proportion of BIPOC instructional staff(\(W\) = 0.815, \(p\) = 0.0001) are statistically significantly different than a normal distribution. The scales assessing the respondent's belonging (\(W\) = 0.977, \(p\) = 0.7373) and the respondent's perception of campus climate for Black students (\(W\) = 0.950, \(p\) = 0.1812) did not differ differently from a normal distribution.
...
\begin{verbatim}
##    vars  n mean   sd median trimmed  mad  min  max range skew kurtosis   se
## X1    1 32 3.71 2.12   3.42     3.6 2.28 0.15 8.13  7.98 0.38    -0.81 0.37
\end{verbatim}
...
## 1      2      2       3      6.33       2.33     3.33      2.83 2.6489551
## 2      4      1       2      4.67       1.67     4.00      2.83 1.5353663
## 3     NA      2      NA        NA       2.00     3.50        NA 0.1480728
## 4      2      2       6      3.33       3.33     2.00      2.67 0.4787930
## 5      1      1       2      4.00       1.33     1.00      1.17 4.0568827
## 6      1      2       3      5.33       2.33     1.00      1.67 5.0889842
##   MOutlier
## 1    FALSE
## 2    FALSE
## 3    FALSE
...
## -1.67164 -0.72382 -0.02559  0.66490  1.66682 
## 
## Coefficients:
##             Estimate Std. Error t value Pr(>|t|)    
## (Intercept)  2.58690    0.65218   3.967 0.000611 ***
## Belonging    0.02799    0.13674   0.205 0.839623    
## cmBlack     -0.02396    0.02160  -1.110 0.278664    
## iBIPOC_pr   -0.36969    0.49354  -0.749 0.461417    
## ---
## Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
## 
## Residual standard error: 0.9877 on 23 degrees of freedom
##   (5 observations deleted due to missingness)
## Multiple R-squared:  0.07555,    Adjusted R-squared:  -0.04503 
## F-statistic: 0.6265 on 3 and 23 DF,  p-value: 0.6052
\end{verbatim}
...
Results of a multiple regression predicting the respondents' perceptions of campus climate for Black students indicated that neither contributions of the respondents' personal belonging (\(B\) = 0.028, \(p\) = 0.840) nor the proportion of BIPOC instructional staff (\(B\) = -0.370, \(p\) = 0.461), nor the proportion of Black classmates (\(B\) = -0.024, \(p\) = 0.279) led to statistically significant changes in perceptions of campus climate for Black students. Although it accounted for 7.55\% of the variance, the overall model was not statistically significant. Results are presented in Table 2.
...
## 
##   Variable     M    SD   1           2           3          
##   1. iBIPOC_pr 0.42 0.40                                    
##                                                             
##   2. cmBlack   7.53 8.61 -.07                               
##                          [-.42, .29]                        
##                                                             
##   3. Belonging 4.24 1.52 .27         -.23                   
##                          [-.11, .58] [-.54, .14]            
##                                                             
##   4. ClimateBL 2.44 0.99 -.14        -.24        -.04       
##                          [-.49, .26] [-.55, .14] [-.40, .33]
##                                                             
## 
...
##  (Intercept) 2.59**  [1.24, 3.94]                                         
##    Belonging   0.03 [-0.25, 0.31]  0.04 [-0.39, 0.48] .00 [-.03, .03]  .05
##      cmBlack  -0.02 [-0.07, 0.02] -0.23 [-0.65, 0.20] .05 [-.11, .21] -.23
##    iBIPOC_pr  -0.37 [-1.39, 0.65] -0.15 [-0.58, 0.27] .02 [-.09, .13] -.14
##                                                                           
##                                                                           
##                                                                           
##              Fit
...
\begin{verbatim}
##    Blst_1 iBIPOC_pr    Blst_4  Belong_1  Belong_3    Blst_2    Blst_3    Blst_5 
##   0.12500   0.06250   0.06250   0.03125   0.03125   0.03125   0.03125   0.03125 
##    Blst_6   cEval_1  cEval_19 
...
##  1 (Intercept)  2.84       0.625     4.55   0.0000960    32
##  2 Belonging   -0.0153     0.126    -0.121  0.905        32
##  3 cmBlack     -0.0296     0.0217   -1.36   0.184        32
##  4 iBIPOC_pr   -0.0778     0.462    -0.168  0.868        32
##  5 (Intercept)  2.88       0.619     4.65   0.0000724    32
##  6 Belonging   -0.0266     0.123    -0.216  0.831        32
##  7 cmBlack     -0.0289     0.0214   -1.35   0.188        32
##  8 iBIPOC_pr   -0.103      0.439    -0.234  0.817        32
##  9 (Intercept)  2.95       0.600     4.91   0.0000353    32
## 10 Belonging   -0.00779    0.127    -0.0614 0.951        32
## 11 cmBlack     -0.0288     0.0214   -1.34   0.189        32
## 12 iBIPOC_pr   -0.418      0.475    -0.880  0.386        32
## 13 (Intercept)  2.89       0.606     4.77   0.0000522    32
## 14 Belonging   -0.0353     0.124    -0.285  0.777        32
## 15 cmBlack     -0.0279     0.0212   -1.32   0.198        32
## 16 iBIPOC_pr   -0.145      0.445    -0.325  0.748        32
## 17 (Intercept)  2.90       0.632     4.59   0.0000859    32
## 18 Belonging   -0.0280     0.126    -0.222  0.826        32
## 19 cmBlack     -0.0281     0.0218   -1.29   0.209        32
## 20 iBIPOC_pr   -0.118      0.447    -0.265  0.793        32
\end{verbatim}
...
## 1 (Intercept) 5  2.89046308 0.3800146853 1.402721e-03 0.3816979504    28
## 2   Belonging 5 -0.02259404 0.0156938264 1.196715e-04 0.0158374322    28
## 3     cmBlack 5 -0.02864923 0.0004623656 4.779758e-07 0.0004629391    28
## 4   iBIPOC_pr 5 -0.17228649 0.2058654746 1.946196e-02 0.2292198257    28
##         df         riv      lambda        fmi
## 1 26.07473 0.004429474 0.004409940 0.07289484
## 2 25.94220 0.009150461 0.009067490 0.07754415
## 3 26.16083 0.001240514 0.001238977 0.06973915
## 4 22.17120 0.113444720 0.101886262 0.17324669
\end{verbatim}
...
## 1 (Intercept)  2.89046308 0.61781708  4.678509 26.07473 7.803438e-05
## 2   Belonging -0.02259404 0.12584686 -0.179536 25.94220 8.589118e-01
## 3     cmBlack -0.02864923 0.02151602 -1.331530 26.16083 1.945013e-01
## 4   iBIPOC_pr -0.17228649 0.47876907 -0.359853 22.17120 7.223622e-01
\end{verbatim}
...
Results of a multiple regression predicting the respondents' perceptions of campus climate for Black students indicated that neither contributions of the respondents' personal belonging (\(B\) = -0.023, \(p\) = 0.859) nor the proportion of BIPOC instructional staff (\(B\) = -0.172, \(p\) = 0.722) led to statistically significant changes in perceptions of campus climate for Black students. Results did suggest a statistically significant negative effect of the proportion of Black classmates on the campus climate for Black students. Specifically, as the proportion of Black students increases, there is less stigmatization and hostility on the campus (\(B\) = -0.029, \(p\) = 0.195). Results are presented in Table X.
...
Data screening suggested that 34 individuals opened the survey link. Of those, 32 granted consent and proceeded to the survey items. A further inclusion criteria was that the course was taught in the U.S; 32 met this criteria. Across cases that were deemed eligible on the basis of the inclusion/exclusion criteria, missingness ranged from 0\% to 36\%. Across the dataset, 3.98\% of cells had missing data and 81.25\% of cases had nonmissing data. At this stage in the analysis, we allowed all cases with fewer than 50\% missing to be included the multiple imputation \citep{katitas_getting_2019}.
...
Results of a multiple regression predicting the respondents' perceptions of campus climate for Black students indicated that neither contributions of the respondents' personal belonging (\(B\) = 0.028, \(p\) = 0.840) nor the proportion of BIPOC instructional staff (\(B\) = -0.370, \(p\) = 0.461) led to statistically significant changes in perceptions of campus climate for Black students. Results did suggest a statistically significant negative effect of the proportion of Black classmates on the campus climate for Black students. Specifically, as the proportion of Black students increases, there is less stigmatization and hostility on the campus (\(B\) = -0.024, \(p\) = 0.279). Although it accounted for 7.55\% of the variance, the overall model was not statistically significant. Results are presented in Table 2.
